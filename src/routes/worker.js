...
module.exports.myDownloadHandler = (req, res) => {
  try {
    ...
    const cmdContent = [
      ...
      ':loop',
      'echo.',
      'echo [%date% %time%] xcipe-worker 시작 (abnormal 재시작 %RESTART_COUNT% 회)',
      'node "%~dp0xcipe-worker.js"',
      'set EXIT_CODE=%errorlevel%',
      'if %EXIT_CODE% equ 0 (',
      '  echo [%date% %time%] 정상 종료 (self-update). 카운터 초기화 후 즉시 재시작.',
      '  set RESTART_COUNT=0',
      '  goto loop',
      ')',
      'set /a RESTART_COUNT+=1',
      'echo [%date% %time%] 비정상 종료 (exit %EXIT_CODE%). 3초 후 재시작...',
      'if %RESTART_COUNT% geq 5 (',
      '  echo.',
      '  echo ===================================================',
      '  echo [FATAL] 5회 연속 워커 비정상 종료. 무한 루프 방지로 중단.',
