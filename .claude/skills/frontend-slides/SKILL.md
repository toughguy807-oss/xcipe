...
- ❌ **사용자 미선택 강행**: visual exploration 생략하고 임의 스타일로 생성 → "내 톤이 아님" 재작업. 옵션 제시 → 사용자 선택 후 진행 의무.
- ❌ **PPT 변환 시 자산 손실**: 이미지/폰트/차트를 base64나 외부 링크 없이 처리 → 산출물 깨짐. 자산은 inline 또는 명시적 폴더 구조로 보존.
- ❌ **반응형 검증 누락**: 발표 환경(프로젝터/노트북) 해상도 다양 → 1080p만 가정 시 4K/720p에서 깨짐. 16:9 기준 + 폰트 최소 18pt + max-width 컨테이너.
- ❌ **애니메이션 과잉**: 모든 슬라이드에 entrance animation 적용 → 발표 흐름 방해 + 대기 시간. signature 1-2개만 일관 적용.
- ❌ **단일 스타일 강요**: "all presentations look like this"식 템플릿 강제 → 비짓강남 vs 솔루션트리 같은 다른 도메인을 동일 톤으로. 도메인별 톤 분기 의무.
- ❌ **빌드 산출물 검증 누락**: 키보드 네비게이션, 슬라이드 카운터, fullscreen mode 미검증 → 실제 발표 시 동작 안 함. 빌드 후 브라우저 직접 테스트.
